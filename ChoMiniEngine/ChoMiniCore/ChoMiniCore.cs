...
{
    public async UniTask RunNode(FlowNode node)
    {
...
{
    private readonly FlowNodeRunner _runner;
    private FlowSequenceFactory _factory;
    public Action OnComplate;
    LocalMessageContext _localMsg;


    public FlowOrchestrator(
        FlowNodeRunner runner)
    {
        _runner = runner;
    }
    ...
    public void SetFactory(FlowSequenceFactory factory) { _factory = factory; }
    ...
    public async UniTask PlaySequence()
    {
        ...
        {
            FlowNode flowNode = _factory.Create();
            nodes.Add(flowNode);
        }
